...
void ObjLoader::createMaterial() 
{
  ...
  std::string path = std::string("m_obj_material.cu.obj");
...
std::cout << "ObjLoader::createGeometryInstances" << std::endl;
  // Load triangle_mesh programs
  if( !m_intersect_program.get() ) {
	  std::cout << "INTERSECT_PROGRAM" << std::endl;
	  //std::cout << sutilSamplesPtxDir();
    std::string path = std::string("m_triangle_mesh.cu.obj");
...
void ObjLoader::loadMaterialParams( GeometryInstance gi, unsigned int index )
{
  ...
  if ( m_material_params.empty() ) {
	...
	gi["bump_map"]->setTextureSampler(loadTexture(m_context, "", make_float3(1.0f, 1.0f, 1.0f) ) );
	gi["usebump"]->setUint(0u);
    return;
  }
  ...
  if ( index < m_material_params.size() ) {
	...
	gi["bump_map"]->setTextureSampler(mp.bump_map );
	gi["usebump"]->setUint(mp.usebump);
    return;
  }
...
void ObjLoader::createMaterialParams( GLMmodel* model )
{
  ...
  for ( unsigned int i = 0; i < model->nummaterials; ++i ) {
	...
	std::string bump_map	 = strlen(mat.bump_map)		? m_pathname + mat.bump_map:"";
	...
	params.bump_map = loadTexture(m_context, bump_map, make_float3(1.0f, 1.0f, 1.0f) );
	params.usebump = strlen(mat.bump_map)? 1 : 0;
	std::cout << mat.name << ((params.usebump)?" uses bump":" doesn't use bump") << std::endl;
  }
}
