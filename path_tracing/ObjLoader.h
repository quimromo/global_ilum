...
public:
   ...
   ~ObjLoader() {} // makes sure CRT objects are destroyed on the correct heap

   void setIntersectProgram( optix::Program program );
   void setBboxProgram( optix::Program program );
   void load();
   void load( const optix::Matrix4x4& transform );

   optix::Aabb getSceneBBox()const { return m_aabb; }
   optix::Buffer getLightBuffer()const { return m_light_buffer; }
